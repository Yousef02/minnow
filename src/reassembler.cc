...
void Reassembler::insert( uint64_t first_index, string data, bool is_last_substring, Writer& output )
{
  ...
  if (ourSet.empty()){
    ...
    if (to_insert.index >= bound->index && to_insert.index + to_insert.data.length() <= bound->index + bound->data.length()) {
      return;
    }
    ...
    while (cur != ourSet.end() && cur->index >= to_insert.index && cur->index + cur->data.length() <= to_insert.index + to_insert.data.length()) {
      setSize -= cur->data.length();
      cur = ourSet.erase(cur);
    }

    // lastly, resolve any partial overlapping at the end of the data being inserted
    if (cur != ourSet.end() && cur->index < to_insert.index + to_insert.data.length() && to_insert.index + to_insert.data.length() < cur->data.length() + cur->index) {
